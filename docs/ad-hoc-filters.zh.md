...
|---|---|---|
| `=` / `!=` | `= 'x'` / `!= 'x'` | 单值等于 / 不等于 |
| `=` / `!=`（多选值） | `IN (...)` / `NOT IN (...)` | 选了多个值时自动转 |
| `=\|`（is one of） | `IN (...)` | 多值「属于」 |
| `!=\|`（is not one of） | `NOT IN (...)` | 多值「不属于」 |
| `<` `<=` `>` `>=` | 同名比较 | 数值/字典序比较 |
| `=~` | `REGEXP` | 正则匹配，值填正则（如 `^/v1/`） |
| `!~` | `NOT REGEXP` | 正则不匹配 |

> ⚠️ **操作符下拉里出现哪些，由 Grafana 内核决定，不是本插件**。插件只负责把 Grafana 传来的
> 操作符翻译成 SQL。下拉列表可滚动——`=~`、`is one of` 等通常在 `>=` 下面。若某版本的
> ad-hoc UI 根本不列出 `=~` / `is one of`，那是 Grafana 限制，可用 `=` 多选值达到 `IN` 的效果。
